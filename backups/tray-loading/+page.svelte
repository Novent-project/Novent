  ...
  <div class="absolute inset-0 z-40 bg-[#08090b] flex flex-col">
    ...
    <div class="flex-1 overflow-y-auto px-6 py-6 flex flex-col gap-6">
      <div class="flex flex-col gap-4">
        <p class="text-[9px] font-mono uppercase tracking-[0.15em] text-white/50">Game Install Paths</p>
        <p class="text-[10px] font-mono text-white/40 leading-relaxed">
        ...
        {#each Object.keys(gamePaths) as key}
          <div class="flex flex-col gap-1.5">
            <label class="text-[10px] font-mono text-white/50 uppercase tracking-widest">{key}</label>
            <input
              type="text"
              ...
              class="w-full bg-black/40 border border-white/[0.08] rounded-md px-3 py-2 text-[11px] font-mono
                     text-white/80 placeholder:text-zinc-700 focus:outline-none focus:border-emerald-500/40 focus:ring-1 focus:ring-emerald-500/25 transition-all"
            />
          </div>
        {/each}
      </div>
      ...
      <div class="flex items-center gap-3">
        <button
          onclick={saveConfig}
          class="px-4 py-2 rounded-md bg-emerald-500/15 border border-emerald-500/30 text-[11px] font-mono
                 text-emerald-300 hover:bg-emerald-500/25 hover:text-emerald-200 transition-all"
        >
          Save
        </button>
        {#if saveStatus}
...
{/if}
    ...
    <header class="flex h-11 shrink-0 items-center justify-between gap-2 border-b border-white/[0.06] px-3 bg-[#0c0d10]/80 backdrop-blur-md">
      ...
      <div class="flex items-center gap-3">
        <div class="flex items-center gap-0.5 rounded-md bg-white/[0.03] border border-white/[0.06] px-0.5 py-0.5">
          <button onclick={() => adjustZoom(-ZOOM_STEP)} class="w-6 h-6 flex items-center justify-center rounded-sm text-zinc-500 hover:text-white hover:bg-white/[0.06] transition-all"><span class="material-symbols-outlined" style="font-size:16px;">remove</span></button>
          <span class="text-[10px] font-mono text-white/50 w-8 text-center tabular-nums">{Math.round(appZoom * 100)}%</span>
          <button onclick={() => adjustZoom(ZOOM_STEP)}  class="w-6 h-6 flex items-center justify-center rounded-sm text-zinc-500 hover:text-white hover:bg-white/[0.06] transition-all"><span class="material-symbols-outlined" style="font-size:16px;">add</span></button>
        </div>
        <Separator orientation="vertical" class="h-4 bg-white/10" />
        <button
