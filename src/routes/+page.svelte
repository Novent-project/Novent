<script lang="ts">
	...
	import { fetchLaps, fetchTelemetry, fetchBoundaries, fetchStatus, fetchConfig } from '$lib/api.js';
	import type { Lap, Telemetry, TrackBoundaries } from '$lib/api.js';
	import { parseLapTime, formatTime } from '$lib/utils/format.js';
	import { downsample, chartWindowFor, LAP_COLORS } from '$lib/canvas/shared.js';
	import type { Trace, DownsampledTrace, ChartWindow } from '$lib/canvas/shared.js';
	import { drawThrottle, drawThrottleSingle } from '$lib/canvas/throttle.js';
	...
	const ZOOM_MIN  = 0.5;
	const ZOOM_MAX  = 2.0;
	const GAME_KEYS = ['AC', 'ACC', 'iRacing', 'LMU'] as const;
	...
	let gamePaths     = $state<Record<string, string>>(Object.fromEntries(GAME_KEYS.map(k => [k, ''])));

	let currentTrace = $state.raw<Trace>({
		gas: [], brake: [], steer: [], normPos: [], worldX: [], worldZ: [], time: [],
	});
	...
	onMount(() => {
		...
		async function setupTray() {
			...
			trayUnlisten = await win.onResized(async () => {
